...
function privateSave(original, active, timeSpan, interestRate) {
    ...
    interestTaxe = (Math.round(interestTaxe * 100)) / 100;
...
function privateSaveHQ(original, active, yearBegin, monthBegin, dayBegin, yearEnd, monthEnd, dayEnd, interestRate) {
    ...
    if (parseInt(n631) > -1) {
        var timeSpan1 = parseFloat(firstYear631) * 12 + parseFloat(7) - parseFloat(yearBegin) * 12 - parseFloat(monthBegin) - parseFloat(dayBegin / 30);
        var timeSpan2 = parseFloat(nowYear) * 12 + parseFloat(nowMonth) + parseFloat(nowDay / 30) - parseFloat(lastYear631) * 12 - parseFloat(7);
        var objArray = new Array();
    ...
    if (parseInt(n631) == -1) {
        var timeSpan = parseFloat(nowYear * 12) + parseFloat(nowMonth) + parseFloat(nowDay / 30) - parseFloat(yearBegin * 12) - parseFloat(monthBegin) - parseFloat(dayBegin / 30);
        var objArray = new Array();
...
function estateBorrow(original, active, timeSpan) {
    ...
    monthBack = (Math.round(monthBack * 10000)) / 10000;
...
function privateborrow1(original, active, timeSpanEvery, termNum) {
    ...
    for (i = 1; i < parseInt(termNum) + 1; i++) {
        interestM = parseFloat(original) / parseInt(termNum) + (parseFloat(original) - parseFloat(original) * (i - 1) / parseInt(termNum)) * active * 0.01 * timeSpanEvery / 12;
        interestTotal = parseFloat(interestTotal) + parseFloat(interestM)
    }
    interestTotal = (Math.round(interestTotal * 100)) / 100;
    var objArray = new Array();
...
function privateborrow2(original, active, timeSpanEvery, termNum) {
    ...
    var interestTotal = 0;
    for (i = 1; i < parseInt(termNum) + 1; i++) {
        interestM = parseFloat(original) / parseInt(termNum) + (parseFloat(original) - parseFloat(original) * (i - 1) / parseInt(termNum)) * active * 0.01 * 0.5 * 2 * timeSpanEvery / 12;
        interestTotal = parseFloat(interestTotal) + parseFloat(interestM)
    }
    interestTotal = (Math.round(interestTotal * 100)) / 100;
    var objArray = new Array();
...
function privateborrow3(original, active, timeSpanEvery, termNum) {
    ...
    var interestTotal = 0;
    for (i = 1; i < parseInt(termNum) + 1; i++) {
        interestM = parseFloat(original) / parseInt(termNum) + (parseFloat(original) - parseFloat(original) * (i - 1) / parseInt(termNum)) * active * 0.01 * 2 * timeSpanEvery / 12;
        interestTotal = parseFloat(interestTotal) + parseFloat(interestM)
    }
    interestTotal = (Math.round(interestTotal * 100)) / 100;
    var objArray = new Array();
