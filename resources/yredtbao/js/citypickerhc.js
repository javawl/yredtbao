	...
	for(var e = 0;e<cityindex.length;e++){
		...
		for(var i = 0;i<LAreaData.length;i++){
			if(LAreaData[i].en_name.substring(0, 1)==cityindex[e]){
				var $creatcity = "<p class='cityper' city='" + LAreaData[i].en_initial +"' name='" +LAreaData[i].en_name.toLowerCase()+"' text='"+ LAreaData[i].cityname+"'>" + LAreaData[i].cityname + "</p>";
				$(".cityindex").eq(e).show();
